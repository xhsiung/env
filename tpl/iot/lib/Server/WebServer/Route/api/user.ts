...
import { Repository as UserRepository } from "../../../../Repository/Mongodb/UserRepository";
/**
 * @swagger
 * tags:
 *   name: User
...
export namespace Route {
  export const routeName = "/api/user";
  export function getRoute() {
    const router = express.Router();
    const userRepository = new UserRepository.Module();
    userRepository.Init().then(() => {
      ...
      router.get("/", async (req, res) => {
        try {
          if (req.session!.user) {
            const user = req.session!.user as UserDomain.IUser;
            user.password = ``;
      ...
      router.post("/auth", async (req, res) => {
        try {
          let request = req.body as WebDomain.ILoginRequest;
          let user = await userRepository.auth(
            request.email,
      ...
      */
      ...
      router.put("/", async (req, res, next) => {
        try {
          const body = req.body as UserDomain.IUser;
          if (body) {
            // 後台檢查
            ...
            if (body.permission === undefined) {
              body.permission = UserDomain.UserPermission.USER;
            }
  ...
  export function passwd(pwd: string) {
    return crypto.createHash("sha256").update(pwd).digest("base64");
  }
  export function RSA(str: string) {
    return crypto.createHash("RSA-SHA512").update(str).digest("base64");
  }
}
